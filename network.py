...
- Soluciona TypeError: 'SimpleListener' object is not callable
- Soluciona RuntimeWarning
- Identidad y Firma DNIe integradas
"""
import asyncio
import socket
import time
...
# --- CLASE LISTENER HÍBRIDA (CORRECCIÓN) ---
class SimpleListener(ServiceListener):
    def __init__(self, network):
        self.network = network
    
    # Métodos estándar de ServiceListener
    ...
    # Método mágico __call__ por si zeroconf lo trata como función
    ...
    async def resolve_async(self, zc, type_, name):
        try:
            ...
            # Ignorar errores de resolución silenciosamente para no ensuciar la consola
            pass
...
class CompleteNetwork:
    ...
    def get_pending_contacts(self): return []
    def get_verified_peers(self): return list(self.contact_book.values())

    def get_network_stats(self):
        return {
            'peers': len(self.discovered),
            'my_fp': self.my_fingerprint
        }
